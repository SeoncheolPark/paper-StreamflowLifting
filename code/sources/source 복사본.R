...
getnbrs_stream3 <- function(remove, pointsin, example_network, I_lines, adjacency, upperExtra=FALSE){
  #프로토타입 3
  r = which(remove==pointsin) #제거할 점의 pointsin에서의 위치
  pointsin_imsi <- pointsin[-r]

  nbrs_upstream <- c()
  nbrs_downstream <- c()
  
  #나중에 pointsin 추가해야 할지도
  network_SegmentID <- example_network@obspoints@SSNPoints[[1]]@network.point.coords$SegmentID[r] #제거할 점의 segment id
  
  n_inthe_Segment <- sum(example_network@obspoints@SSNPoints[[1]]@network.point.coords$SegmentID==network_SegmentID) #network_SegmentID에 들어있는 관찰값의 수
  r_length <- nchar(adjacency$rid_bid[network_SegmentID,2]) #제거 점의 rid_bid의 character 글자 수
  
  #index <- setdiff()
  
  network_indexID <- example_network@obspoints@SSNPoints[[1]]@network.point.coords$SegmentID[-r] #나머지 이웃대상 점들의 segment ID (3,5,4)
  #my addition(2019년 3월 12일)
  network_SegmentID <- as.numeric(levels(network_SegmentID))[network_SegmentID]
  network_indexID <- as.numeric(levels(network_indexID))[network_indexID]
  ...
  upstream_index <- str_which( substr(adjacency$rid_bid[network_indexID,2], start=1, stop=r_length), adjacency$rid_bid[network_SegmentID,2])
  #upstream_index <- setdiff(pointsin_imsi[str_which( substr(adjacency$rid_bid[network_indexID,2], start=1, stop=r_length), adjacency$rid_bid[network_SegmentID,2])], remove)
  upstream_index <- setdiff(upstream_index, samestream_index)
  
  #upstream_index <- pointsin_imsi[upstream_index]
  ...
  if(length(nbrs_upstream)==0 & length(upstream_index)!=0){
    ...
    cand_length <- nchar(adjacency$rid_bid[network_indexID,2])
    nbrs_cand <- which(nchar(adjacency$rid_bid[network_indexID,2]) > nchar(adjacency$rid_bid[network_SegmentID,2]))
    ...
    while(TRUE){
      ...
      #upstream_candidate < adjacency$rid_bid[network_indexID[nbrs_cand],2]- nbrs_cand[which(diff_vec==min(diff_vec))]
      ...
      nbrs_upstream <- c(nbrs_upstream, pointsin_imsi[upstream_candidate])
...
streamPred <- function(pointsin, X=example_network, coeff=sims@obspoints@SSNPoints[[1]]@point.data$Sim_Values, nbrs, remove, adj=adjacency, method=c("IDW","LC"), intercept=FALSE, neighbours=NULL){
...
importSSN_stream <- function(shreve_obj){
  ...
  shape <- data$shape
  #2012년부터 자를까
  data$eventdata <- data$eventdata["2012/2017",]
...
get_binaryIDs_stream <- function(mouth_node, shape){
