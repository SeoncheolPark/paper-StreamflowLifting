  ...
  if(segment.num%in%points.segmentID){
    ...
    if(length(upper_candidate_index)!=0){
      while(TRUE){
        ...
        nbrs_upper_candidate2 <- sort(unique(c(nbrs_upper_candidate, upper_imsi_result2[[1]])))
        segs_upper_candidate2 <- sort(unique(c(segs_upper_candidate, upper_imsi_result2[[2]])))
        remove_index_imsi2 <- sort(unique(c(remove_index_imsi,upper_imsi_result2[[3]])))
        nbrs_upper_candidate <- sort(unique(c(nbrs_upper_candidate, upper_imsi_result2[[1]])))
        segs_upper_candidate <- sort(unique(c(segs_upper_candidate, upper_imsi_result2[[2]])))
        remove_index_imsi <- sort(unique(c(remove_index_imsi,upper_imsi_result2[[3]])))
    ...
    #    print(nbrs_upper_candidate2)
        ...
        #error처리
    #    if(length(nbrs_upper_candidate)!=0 & length(nbrs_upper_candidate2)!=0){
    #      if(!all.equal(sort(nbrs_upper_candidate),nbrs_upper_candidate2)){
    #        stop(paste("nbrs_upper_candidate does not match: ", nbrs_upper_candidate, nbrs_upper_candidate2))
    #      }
    #    }
    #    if(length(segs_upper_candidate)!=0 & length(segs_upper_candidate2)!=0){
    #      if(!all.equal(sort(segs_upper_candidate),segs_upper_candidate2)){
    #        stop(paste("segs_upper_candidate does not match: ", segs_upper_candidate, segs_upper_candidate2))
    #      }
    #    }
    #    if(length(remove_index_imsi)!=0 & length(remove_index_imsi2)!=0){
    #      if(!all.equal(sort(remove_index_imsi),remove_index_imsi2)){
    #        stop(paste("remove_index_imsi does not match: ", remove_index_imsi, remove_index_imsi2))
    #      }
    #    }
    #    
    #    remove_index <- sort(unique(c(remove_index_imsi, upper_index_imsi)))
     #   upper_candidate_index <- setdiff(upper_candidate_index, remove_index)
    #    if(length(upper_candidate_index)==0){
    #      break
    #    }
      }
...
getnbrs_fast_internal <- function(upper_index_imsi, pointsin, points.segmentID, example_network, adjacency, upper_candidate_index){
  nbrs_upper_candidate <- c()
  remove_index_imsi <- c()
  segs_upper_candidate <- c()
