...
#수정 (Oct 18, 2020)
# 이 코드를 주석처리 하면 원래 논문의 tilde{RMSE} 결과가 나옴
...
########################################
#2nd revision 관련(April 28, 2021)
########################################
#qqnorm(data-data_predicted0[as.numeric(names(data))])
#qqline(data-data_predicted0[as.numeric(names(data))], col=2, lty=2, lwd=2)
#pdf("residual_TOCFit.pdf", 4.5, 4.5)
#png("residual_TOCFit.png", 450, 450)
#pdf("residual_TOCFit(2).pdf", 8.5, 4.25)
#png("residual_TOCFit(2).png", 850, 425)
#par(mfrow=c(1,2))
par(mfrow=c(1,1))
#qqnorm(data-data_predicted1[as.numeric(names(data))], main="(a) ODonell")
#qqline(data-data_predicted1[as.numeric(names(data))], col=2, lty=2, lwd=2)
#qqnorm(data-data_predicted[as.numeric(names(data))], main="(a) S-Lifting (M)")
#qqline(data-data_predicted[as.numeric(names(data))], col=2, lty=2, lwd=2)
#qqnorm(data-data_predicted2[as.numeric(names(data))], main="(b) S-Lifting (N)")
#qqline(data-data_predicted2[as.numeric(names(data))], col=2, lty=2, lwd=2)
qqnorm(data-data_predicted2[as.numeric(names(data))], main="S-Lifting (N)")
qqline(data-data_predicted2[as.numeric(names(data))], col=2, lty=2, lwd=2)
dev.off()
par(mfrow=c(2,2))

########################################
#2nd revision 관련 끝(April 28, 2021)
########################################
...
result_list$dataname <- names(data)

result_list$result_forward <- result_forward
result_list$result_denoise <- result_denoise
result_list$result_nlt <- result_nlt
result_list$result_ODonell <- result_7
result_list$denoising_ODonell <- denoising_ODonell
result_list$predicted_raw <- data_predicted0
result_list$predicted_ODonell <- data_predicted1
result_list$predicted_mlift <- data_predicted
result_list$predicted_nlift <- data_predicted2
result_list$result_new_mat <- result_new_mat
saveRDS(result_list, "GeumRiver(modified).RDS")
